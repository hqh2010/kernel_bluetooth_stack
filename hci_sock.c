			    ...
			    size_t len, int flags)
{
	...
	BT_INFO("tttt hci_sock_recvmsg sock %p, sk %p", sock, sk);
			...
			struct msghdr *msg, size_t msglen)
{
	...
	BT_INFO("tttt hci_mgmt_cmd %d channel:%d", index, chan->channel);
	...
	cp = buf + sizeof(*hdr);
	if (hdev)
	BT_INFO("tttt hci_mgmt_cmd index:%d name:%s opcode:%d", index, hdev->name, opcode);
			    ...
			    size_t len)
{
	...
	BT_DBG("tttt hci_sock_sendmsg sock %p sk %p", sock, sk);
